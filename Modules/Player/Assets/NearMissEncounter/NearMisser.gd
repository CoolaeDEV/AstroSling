		...
		if not player.crashed:
			...
			if closestPlanet:
				if (closestPlanet.position - player.position).length() <= 250:
					...
					var PArrowScale = 0.05 + (PArrowScaleFactor * (0.4 - 0.05))
					...
					var PlanetArrowScale = 0.05 + (PlanetArrowScaleFactor * (0.4 - 0.05))
					...
					var positionOnSphere = closestPlanet.global_position + dirToPlayer * (closestPlanet.Radius + 80)
					...
					distanceLabel.global_position = (player_distance_node.global_position + planet_distance_node.global_position) / 2
